...
In his book, Wheelan claims that (a) for 10 blind tast testers, the probability of a happy outcome is 0.83 and (b) for 100 blind taste testers, the probability is 0.98. If you don't want to take this assertion at face value, you have two options. One is to use the simulation tool to run 100 simulated experiments of sample size 10 or 100 and inspect the proportion of those experiments that led to a favorable outcome. You should see values around .83 and .98 for sample sizes of 10 and 100, respectively. Your second option is to do the probability calculation by hand.   

To find the empirical (i.e., exact) answer, instead of using a simulation to estimate it, it might be helpful to consider a smaller sample size, say 2. Now that we've reduced our scope, we have some hope of writing all possible outcomes of this experiment and their probabilities.  

Ok. So what are all of the possible outcomes of 2 independent taste tests?  There are two possibilities for each taste tester (they can choose either Schlitz or Michelob), so if we represent each possible outcome as (taste-tester 1's choice, taste-tester 2's choice), we get 4 possibilities (in many probability textbooks, possible outcomes of a random experiment are represented as sets, which are enclosed in curly brackets): {(Michelob, Michelob), (Michelob, Schlitz), (Schlitz, Michelob), (Schlitz, Schlitz)}. This is also called the **sample space** of the random experiment.    
...
Finally, if we want to account for different probabilities of preferring Schlitz or Michelob, it's helpful to add one more tool to the toolbox: the binomial probability distribution. By definition, dbinom(x,n,p) gives the probability of x "successes" in n independent random trials, where each random trial has probability of success=p. For example, dbinom(4,10,.5) could be thought of as the probability of exactly 4 out of 10 people preferring Schlitz if the probability of any individual preferring Schlitz is .5. Using this function, we can now repeat the calculation above using some slightly different code:  
...
All of these calculations might feel a little overwhelming and confusing at this point. If that's the case, don't fear! We have another option. For a moment, let's pull back the curtain on the Schlitz simulation and see what's going on under the hood. The following code walks you through the process of repeatedly randomly surveying some number of people and calculating the proportion of those surveys that led to acceptable outcomes. To start, we can assume that the sample size is 10, the probability of preferring Schlitz is .5, and we want the probability of at least 40% of taste-testers preferring Schlitz. Note that, if we take a lot (i.e., 10,000) random samples of 10 people, we end up with almost the same probability calculation as we did before. We could be even more confident if we took 100,000 or even 1,000,000 random samples, and this takes less than a second on most computers.:    
...
Feel free to copy this code over into your own R Studio window, and play with the parameters to see what happens. If nothing else, I hope you feel some satisfaction at the idea that we were able to answer the same question in 3 different ways. Simulation is a powerful tool that allows us to side-step some pretty advanced probability calculations!   
...
In this book, I have tried to emphasize conceptual understanding through simulation and discussion. In the example above, you can, for example, try to experiment with the Schlitz simulation to get a feel for the outcome. But "roughly 75%" based on a few experiments is not a precise statement of confidence. If this bothers you, read on.

Mathematical statistics does have precise answers that depend on properties of continuous distributions like the normal distribution and the binomial distribution. The ice cream test is exactly the kind of scenario that is explained using a binomial distribution. If we ran the simulation (always with samples of 10), taking more and more observations (i.e., 10 samples of 10, 100 samples of 10, etc.) and checked our success rate (defined by more than 5 prefering mint chocolate chip), we would see that indeed this proportion does converge. This is plotted in Figure \@ref(fig:converge-binom). The x-axis is the number of samples, but **the x-axis is shown using logarithmic scales**. We need to use this scale, or else all of the points at smaller values would be bunched together.
...
for (n in n_obs) {
   ...
   obs_prop <- c(obs_prop, length(which(tmp > 5))/length(tmp))
}
plot(n_obs, obs_prop, 
...
We say that the distribution of sample proportions is binomial in the limit of large numbers of samples. It's not going to be perfect for 5 samples or 20, but if we had an infinite number... we say that it asymptotically approaches the binomial distribution. The abstraction to continuous distributions makes it possible to compute "exact" probabilities. For example the horizontal dashed-line in Figure \@ref(fig:converge-binom) occurs at this value:
...
sum(dbinom(min_success:sample_size,sample_size,prob_success))

```
The one line of R code was all that was necessary to calculate this asymptotic value. The function `dbinom()` returns the **probability density function** of the binomial distribution. The density has a value for each of the discrete possibilities, between 0 and 10 in this case, of "how many people prefer mint chocolate chip." I've also specified that the assumed probability for any one person is 0.65. Finally, I was interested in all of the outcomes in {6,7,8,9,10} for which you win the bet. 

> 2 cent bonus: As a reward for having read to this point, you may now see that actually your asymptotic probability of winning the bet is a little better than 0.75 or 75%. It's actually 0.7514955! That means, your expected value is actually not 0, but 0.018.
