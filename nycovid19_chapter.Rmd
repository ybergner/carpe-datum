...
boro_over65_proportions <- data.frame(countyName =acs_boro_over65$NAME,
                                      Borough=unname(getBoro[acs_boro_over65$countyName]), 
...
As mentioned above, it is possible to query the Census for zipcode tabulation areas. You need to specify which zipcodes, so we will get our list from the NYC data. 
...
```{r}
names(zcta_income) <- c("zipcode","medianHHincome", "medianIncome")
head(zcta_income)
```
...
While we're at it, we can keep collecting variables related to the reports we read. For example, household size seems to be associated with prevalence of COVID-19 and so does the share of college-educated adults (recall the image at the beginning of this chapter). The ACS5 variables we will need are:
...
Having gathered the information in various tables ()
