...
zcta_join <- zcta_income %>% 
  ...
  left_join(nyc_zipcode_data %>% mutate(zipcode = as.character(zipcode)), by=c("zipcode"))
...
```{r abc-college, out.width='80%', fig.cap='ABCNews visualization'}
...
par(mfrow=c(1,3))
plot(NYC_zips, col = myShapeColorsCovid$regionValues, main = "covid", cex = 10, lwd=0.05)
plot(NYC_zips, col = myShapeColorsCollege$regionValues, main = "college", cex = 10, lwd=0.05) 
plot(NYC_zips, col = myShapeColorsCovid$regionValues, main = "covid+college", cex = 10, lwd=0.05)
plot(NYC_zips, col = myShapeColorsCollege$regionValues, add=T, cex = 10, lwd=0.05)
...
par(mfrow=c(1,3))
plot(NYC_zips, col = myShapeColorsCovid$regionValues, main = "covid", cex = 10, lwd=0.05)
plot(NYC_zips, col = myShapeColorsHH$regionValues, main = "hh size", cex = 10, lwd=0.05) 
plot(NYC_zips, col = myShapeColorsCovid$regionValues, main = "covid+hhsize", cex = 10, lwd=0.05)
plot(NYC_zips, col = myShapeColorsHH$regionValues, add=T, cex = 10, lwd=0.05)
...
plot(mapData$AvgHH, covidPerCapita, col=myShapeColorsCovid$regionValues, pch=16,
     xlab="Avg Household Size", ylab="COVID rate")
points(mapData$AvgHH, covidPerCapita, col=myShapeColorsHH$regionValues, pch=16)
...
plot(mapData$Bach_plus_prop, covidPerCapita, col=myShapeColorsCovid$regionValues, pch=16,
     xlab="% College", ylab="COVID rate")
points(mapData$Bach_plus_prop, covidPerCapita, col=myShapeColorsCollege$regionValues, pch=16)

```

The scatterplots on the right make it easy to see the trends. As average household sizes go up per zipcode, so do COVID rates. As college education goes up, COVID rates go down. The coloring of the points is the same as the coloring of the region. Along the 1-to-1 line (i.e., a perfect positive relationship), we would have all purple points of different shades. The farther away we are from this diagonal, the more red (above the line) or blue (below) the points are.
