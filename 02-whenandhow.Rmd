...
The first thing you might do is tabulate the data to get the following contingency table:  
|   | Cancer: Yes| Cancer: No |
|---|:-:|:-:|
| Smoke: Yes | 46 | 204 |
| Smoke: No | 93 | 657 |  

As a first step toward understanding whether there is an association between smoking and cancer, you could then estimate the following:  
...
One way to *start* trying to answer this question is to consider the following thought experiment: imagine that, among all people in the world, there is NOT a higher incidence of cancer among smokers (as compared to non-smokers). If that were the case, you would expect to see $P(\text{Cancer}|\text{Smoke})=P(\text{Cancer}|\text{Not Smoke})=.5$. Or, written slightly differently:  $P(\text{Cancer}|\text{Smoke})-P(\text{Cancer}|\text{Not Smoke})=0$. In comparison, you observed the following in your initial sample: $P(\text{Cancer}|\text{Smoke})-P(\text{Cancer}|\text{Not Smoke})=0.184-0.124=0.06$.   

So, you could pose the following question: what is the probability that, among the whole population, smokers do not have increased risk of cancer; but, among the random sample of 1000 people that you observed, there is a 6% (or greater) increased incidence of cancer among smokers as compared to non-smokers? This type of question is the basis for **hypothesis testing**. If you are interested in learning more about how statisticians use probability distributions to answer these types of questions, there are plenty of resources online. In this book, however, we will attempt to answer this question using a simulation in R.  

Based on the sample we observed, we could estimate that approximately $\frac{46+204}{1000}*100=25$ percent of the population smokes and approximately $\frac{46+93}{1000}*100=13.9$ percent of the population has been diagnosed with cancer. Therefore, we could create some fake datasets in R where each person has a 25% chance of smoking and, independently, a 13.9% chance of getting diagnosed with cancer. Then, for each of these fake datasets, we could calculate $P(\text{Cancer}|\text{Smoke})-P(\text{Cancer}|\text{Not Smoke})$ and calculate the proportion of these differences that are greater than or equal to $0.06$:  
...
You might notice that a very small proportion of the simulated values of $P(\text{Cancer}|\text{Smoke})-P(\text{Cancer}|\text{Not Smoke})$ were greater than or equal to .06. (about .03 or 3%). This might help convince you that there was a relatively low probability of observing such an extreme value of $P(\text{Cancer}|\text{Smoke})-P(\text{Cancer}|\text{Not Smoke})$ among your sample of 1000 people if the reality was that $P(\text{Cancer}|\text{Smoke})-P(\text{Cancer}|\text{Not Smoke})=0$ among the full population.  

That said, there are two problems: 1) there is still some risk that there is no real difference in cancer risk among these two groups, but we observed one nonetheless and 2) even if there is a real difference, there are a lot of possible reasons that this different could exist without a direct causal relationship.  
